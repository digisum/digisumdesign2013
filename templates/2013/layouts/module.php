...
if (in_array($module->position, array('footer'))) {
	$style = '';
	$showtitle = 0;
}
...
switch ($style) {
	...
	case 'box':
		$template		= 'default-1';
		$style			= 'mod-'.$style;
		$style			.= ($color) ? ' mod-box-'.$color : ' mod-box-default';
		$style			.= (strpos($color, 'transparent') !== false) ? ' mod-box-transparent' : '';
		$style			.= (strpos($color, 'color') !== false) ? ' mod-box-colored' : '';
...
if (in_array($module->position, array('top-a','top-b','bottom-a','bottom-b','bottom-c')) && $this['config']->get($module->position.'-bg') == "bg-white" &&  (strpos($color, 'transparent') !== false)) {
    $style = str_replace(array("mod-box-transparent10","mod-box-transparent15","mod-box-transparent20","mod-box-transparent25"), "mod-box-default", $style);
    $style = str_replace("mod-box-transparent", "", $style);
} elseif (in_array($module->position, array('innertop','innerbottom','sidebar-a','sidebar-b')) && $this['config']->get('main-bg') == "bg-white" &&  (strpos($color, 'transparent') !== false)) {
    $style = str_replace(array("mod-box-transparent10","mod-box-transparent15","mod-box-transparent20","mod-box-transparent25"), "mod-box-default", $style);
    $style = str_replace("mod-box-transparent", "", $style);
} 
...
if ($module->menu) {
	...
	if (isset($params['menu'])) {
		...
		$renderer = 'dropdown';
	} else if (in_array($module->position, array('footer'))) {
		$renderer = 'default';
	} else {
		$renderer = 'accordion';
	}
